...
class CustomSNLI(data.TabularDataset):

    urls = ['http://nlp.stanford.edu/projects/snli/snli_1.0.zip']
    dirname = 'snli_1.0_entail'
    name = 'snli'
    ...
    @classmethod
    def splits(cls, text_field, label_field, reduced=False, parse_field=None, root='.data',
               train='snli_1.0_train_entail.jsonl', validation='snli_1.0_dev_entail.jsonl',
               test='snli_1.0_test_entail.jsonl'):
        ...
        if reduced:
            train = '%s_reduced'%train
            validation = '%s_reduced'%validation
            test = '%s_reduced'%test

        if parse_field is None:
            return super(CustomSNLI, cls).splits(
